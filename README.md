...
For the reviewing process, we have also created a Virtual Machine Disk available
[here](https://www.dropbox.com/s/ptfik56d4qphpz7/lubuntu.vdi.gz?dl=0) (login=password=test). Maude is already installed there (just
type `maude`) and the present repository is cloned in `/home/test/RESPEG`. Please, do
a `svn pull` to bring the last version of the files. 
...
### peg-syntax.maude
This functional theory defines the syntax for building parsing expression, grammars and strings (see Sections 2 and 3 of our paper). 
...
Following the mechanism proposed in our paper (Section 4) for throwing and capturing errors in PEGs, valid expressions include also:
...
conditional rules for given meaning to choices, sequential composition and negation. 
...
is `"b" "c"`, corresponding to the string (`bc`) not consumed by the expression. The
...
In this rewrite theory, sequential composition, choices, negation and replication are encoded as suitable constructors on states (more details on Section 3.1 of our paper). For instance, sequential composition is encoded as follows: 
...
The function `parse` receives as parameter the grammar, the initial expression and the string to be parsed. The final state `{eps ; 48}` tells us that all the string was consumed and `48` rules where applied to reach that state. 
On the other hand, the final state `fail(46)` indicates that the input string cannot be matched and the parser failed after `46` steps. 
...
As a simple example, the following grammar fails with an `error` if the string starts with `a` but it does not continue with `bc`. On input `abx`, the failure on `x` produces an error and the second alternative of the choice can be safely ignored (more details on Section 4 of our paper). 
